...
import { Search, RefreshCw, Plus, Filter, MailOpen, Clock, FileCheck, AlertTriangle, ArrowUpRight, ChevronDown, ChevronRight } from "lucide-react";
...
const CourriersManagementPage = () => {
  ...
  const getFilteredCourriers = () => {
    ...
    switch (activeTab) {
      ...
      case 'pending':
        return courriers.filter(c => c.statut === 'RECEIVED' || c.statut === 'SENT');
      case 'processed':
  ...
  const getTabIcon = (tab: string) => {
    switch (tab) {
      case 'inbound': return <MailOpen className="h-4 w-4 mr-1" />;
      case 'outbound': return <ArrowUpRight className="h-4 w-4 mr-1" />;
      case 'urgent': return <AlertTriangle className="h-4 w-4 mr-1" />;
      case 'overdue': return <Clock className="h-4 w-4 mr-1" />;
      case 'processed': return <FileCheck className="h-4 w-4 mr-1" />;
      default: return null;
    }
  };
    ...
    <Container maxWidth="full" className="py-6">
      <div className="flex flex-col space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Gestion des Courriers</h1>
            <p className="text-muted-foreground mt-1">Consultez, filtrez et gérez tous vos courriers</p>
          </div>
          <Button 
            size="lg"
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <Card className="bg-blue-50/50 backdrop-blur">
            <CardContent className="p-4 flex flex-col items-center justify-center">
              <p className="text-sm font-medium text-blue-700">Entrants</p>
          ...
          <Card className="bg-purple-50/50 backdrop-blur">
            <CardContent className="p-4 flex flex-col items-center justify-center">
              ...
              <p className="text-3xl font-bold text-purple-700">{stats.outbound}</p>
            </CardContent>
          </Card>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={toggleFilters} className="flex items-center gap-1">
                  ...
                  Filtres
                  {showFilters ? <ChevronDown className="h-3 w-3 ml-1" /> : <ChevronRight className="h-3 w-3 ml-1" />}
                </Button>
              </div>
            </div>
            ...
            {showFilters && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
                  ...
                  <DatePicker
                    selected={filters.date_envoi ? new Date(filters.date_envoi) : null}
                    onSelect={(date: Date | null) => handleDateFilter('date_envoi', date)}
                  ...
                  <DatePicker
                    selected={filters.date_reception ? new Date(filters.date_reception) : null}
                    onSelect={(date: Date | null) => handleDateFilter('date_reception', date)}
                ...
                {Object.keys(filters).length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {Object.entries(filters).map(([key, value]) => (
                      ...
                      >
                        <span>{key.replace('_', ' ')}: {String(value).substring(0, 15)}</span>
            ...
            <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="px-6 pt-4">
                <TabsList className="w-full grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-1">
                  ...
                  <TabsTrigger value="overdue" className="flex items-center justify-center">
                    {getTabIcon('overdue')}
                    En retard
                    <Badge variant="outline" className="ml-2 bg-slate-100 text-slate-700">
                      {stats.overdue}
                  ...
                  <TabsTrigger value="processed" className="flex items-center justify-center">
                  ...
                  <div className="space-y-4">
                    {isLoading ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-lg font-medium">Erreur de chargement des données</p>
                        <p className="text-muted-foreground max-w-md mt-2">
                          Le format des données reçues est incorrect. Veuillez réessayer ou contacter le support.
                        </p>
                        <Button onClick={refetch} className="mt-6" variant="default">
                      ...
                      <div className="flex flex-col items-center justify-center py-16 text-center">
                        ...
                        <p className="text-muted-foreground max-w-md mt-2">
                          Modifiez vos critères de recherche ou ajoutez un nouveau courrier pour commencer.
                        </p>
